...
void mac_ston(const char* str, u_char mac[ETH_ALEN])
{
...
void mac_ntos(const u_char mac[ETH_ALEN], char* str)
{
...
void Create_ARP_Packet(struct myStruct mystruct, bool packet_type, const u_char sender_mac[ETH_ALEN],
    ...
    memcpy((arp_hdr->sender_ip), &(target_ip->s_addr), sizeof(arp_hdr->sender_ip));
    MEMCPY(arp_hdr->target_mac, sender_mac, sizeof(arp_hdr->target_mac)); //target_mac before
    memcpy((arp_hdr->target_ip), &(sender_ip->s_addr), sizeof(arp_hdr->target_ip));    
    ...
    if(pcap_sendpacket(mystruct.handle, frame, sizeof(frame)) == -1) {
         fprintf(stderr, "%s\n", pcap_errbuf);
