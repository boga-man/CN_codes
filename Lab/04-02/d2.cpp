...
int main(){
    // socket server
    cout<<getpid()<<endl;
    cout<<"[ds2] - Data Server 2 started\n";
    int PORT = 8082;
    int sfd, nsfd, valread;
    struct sockaddr_in address;
    int addrlen = sizeof(address);
    char buffer[1024] = {0};
    strcpy(buffer, "Hello from Dataserver 2\n");
    int opt = 1;

    // creating a socket endpoint
    if ((sfd = socket(AF_INET, SOCK_STREAM, 0)) == 0)
    {
        perror("[ds2] - socket failed");
        exit(EXIT_FAILURE);
    }
    ...
    cout<<"[ds2] - Waiting for connection\n";
    ...
    cout<<"[ds2] - Multi data server connected, waiting for request"<<endl;
