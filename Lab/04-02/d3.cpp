...
int main(){
    // socket server
    cout<<getpid()<<endl;
    cout<<"[ds3] - Data Server 3 started\n";
    int PORT = 8083;
    int sfd, nsfd, valread;
    struct sockaddr_in address;
    int addrlen = sizeof(address);
    char buffer[1024] = {0};
    strcpy(buffer, "Hello from Dataserver 3\n");
    int opt = 1;

    // creating a socket endpoint
    if ((sfd = socket(AF_INET, SOCK_STREAM, 0)) == 0)
    {
        perror("[ds3] - socket failed");
        exit(EXIT_FAILURE);
    }
    ...
    cout<<"[ds3] - Waiting for connection\n";
    ...
    cout<<"[ds3] - Multi data server connected, waiting for request"<<endl;
