...
int main(){
    // socket server
    cout<<getpid()<<endl;
    cout<<"[ds1] - Data Server 1 started\n";
    int PORT = 8081;
    int sfd, nsfd, valread;
    struct sockaddr_in address;
    int addrlen = sizeof(address);
    char buffer[1024] = {0};
    strcpy(buffer, "Hello from Dataserver 1\n");
    int opt = 1;

    // creating a socket endpoint
    if ((sfd = socket(AF_INET, SOCK_STREAM, 0)) == 0)
    {
        perror("[ds1] - socket failed");
        exit(EXIT_FAILURE);
    }
    ...
    cout<<"[ds1] - Waiting for connection\n";
    ...
    cout<<"[ds1] - Multi data server connected, waiting for request"<<endl;
    // continuously send the data to the multi data server on its request
    while (1)
    {
        cout<<"[ds1] - Waiting for a request from the multi server\n";
        // read the data from the multi data server
        char c;
        valread = read(nsfd, &c, 1);
        cout<<"[ds1] - New request from multi server: "<<c<<endl;
