...
{
    int no_pforms = 3;
    int tsfd[3] = {0}, psfd, psfds[no_pforms], maxfd = 0;
    ...
    while (1)
    {
        ...
        {
            int idx;
            if (FD_ISSET(tsfd[i], &rset) && (idx = get_avail_pform(available_platform, no_pforms)) >= 0)
            {
                available_platform[idx] = 0;
                if (nsfd = (accept(tsfd[i], (struct sockaddr *)&taddr, (socklen_t *)&len)) == -1)
                {
                    perror("accept");
                    exit(1);
                }
                ...
                // passing this train through the platform which is free
                // int c = idx;
                // if (send(nsfd, &c, 1, 0) <= 0)
                // {
                //     cout << "Disconnected from train\n";
                //     continue;
                // }

                // if(recv(nsfd, &c, 1, 0) <= 0){
                //     cout<< "Disconnected from train\n";
